/****** Object:  StoredProcedure [Search.Framework].[ResolveURL]    Script Date: 12/13/2012 15:07:19 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		END		
		if(@Param1='concept' and @Param2='')
		BEGIN
			SELECT @Resolved = 1,
				@ErrorDescription = '',
				@ResponseURL = @REDIRECTPAGE + '?tab=concept'
		END		


		if(@Param1='concept' and @Param2='results')
		BEGIN          
		
			SELECT @Resolved = 1,
				@ErrorDescription = '',
				@ResponseURL = @REDIRECTPAGE + '?tab=concept&action=results'
