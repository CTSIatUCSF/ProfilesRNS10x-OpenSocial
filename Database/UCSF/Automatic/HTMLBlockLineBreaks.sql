...
SELECT 841
      ...
      ,[_NumberOfNodes]
      ,[_NumberOfTriples]
  FROM [Ontology.].[ClassProperty] where ClassPropertyID = 820;

-- run this thing to update derived fields
exec [Ontology.].[UpdateDerivedFields];

-- check work
select * from [Ontology.].[ClassProperty] where ClassPropertyID in (820, 841);
...
'<Module ID="HTMLBlockLineBreaks"><ParamList><Param Name="DataURI">rdf:RDF[1]/rdf:Description[1]/vivo:freetextKeyword</Param></ParamList></Module>' as XML) where ClassPropertyID in (841);

UPDATE [Ontology.].[ClassProperty] set CustomDisplay = 1, CustomDisplayModule = cast(
'<Module ID="HTMLBlockLineBreaks"><ParamList><Param Name="DataURI">rdf:RDF[1]/rdf:Description[1]/vivo:overview</Param></ParamList></Module>' as XML) where ClassPropertyID in (820);

UPDATE [Ontology.].[ClassProperty] set MaxCardinality = null, CustomDisplayModule = null where ClassPropertyID in (820, 841);
